	...
	if( isset($_POST['btn-login']) ) {	
		...
		if (!$error) {
		    ...
		    */
			...
			$dt=date('Y-m-d H:i:s');
			$query2 = mysql_query("SELECT * FROM magazzino where id = ".$_SESSION['idProdotto']);
	        $prodotto=mysql_fetch_array($query2);
			$query1=mysql_query("INSERT INTO `movimento`( `idProdotto`, `idUtente`, `nomeProdotto`, `prezzo`, `quantitaPrima`, `quantitaDopo`, `dataMovimento`) VALUES (".$_SESSION['idProdotto'].",".$_SESSION['user'].",'".$nomeProdotto." ".$prodotto['brand']."',0,".$prodotto['quantita'].",$quantita,'".$dt."');");
