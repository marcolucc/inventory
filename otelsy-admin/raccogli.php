...
function buycott_com($codiceBarre){
    ...
    if ($esiste==true){
        ...
        if($get === FALSE) { return "ERROR"; }
            ...
            $query = "INSERT INTO `prodotto`(`codiceBarre`, `nomeProdotto`, `brand`, `produttore`, `indirizzoProduttore`, `prezzo`) VALUES (".$codiceBarreRicevuto.",'".$nomeProdottoRicevuto."','','".$produttore."','".$indirizzoProduttore."','0,00')";
...
$prodotto = buycott_com($codiceBarre);

if (is_null($prodotto[1])  ){
    $prodotto = blia_it($codiceBarre);
    if(is_null($prodotto[1])){
        $prodotto = digiteyes_com($codiceBarre);
...
if($prodotto[1]=="R"){
    ...
    $_SESSION['nomeProdotto']=$prodotto[1];
    echo $_SESSION['nomeProdotto'];
    $_SESSION['brand']=$prodotto[2];
    $_SESSION['produttore']=$prodotto[3];
    $_SESSION['indirizzoProduttore']=$prodotto[4];
    header("Location: aggiungiQuantitaScan.php");
}
